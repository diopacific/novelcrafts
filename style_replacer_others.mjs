...
function replaceTheme(filePath) {
    ...
    // Panels & Backgrounds
    code = code.replace(/bg-white border-slate-200/g, 'bg-[#0f172a]/80 backdrop-blur-xl border-slate-700/50');
    code = code.replace(/bg-white border-b border-slate-200/g, 'bg-[#0f172a]/80 backdrop-blur-xl border-b border-slate-700/50');
    code = code.replace(/bg-white\/70 backdrop-blur-md/g, 'bg-[#0f172a]/70 backdrop-blur-md border border-slate-700/50');
    code = code.replace(/bg-white\/80 backdrop-blur-md/g, 'bg-[#0f172a]/80 backdrop-blur-md border border-slate-700/50');
    code = code.replace(/bg-slate-50\/50 backdrop-blur-sm/g, 'bg-slate-800/50 backdrop-blur-sm border border-slate-700/50');
    code = code.replace(/bg-white border-r border-slate-200/g, 'bg-[#0f172a]/80 backdrop-blur-xl border-r border-slate-700/50');
    ...
    code = code.replace(/bg-indigo-50 text-indigo-700/g, 'bg-indigo-900/50 text-indigo-300');
    code = code.replace(/border-indigo-200/g, 'border-indigo-700/50');
    code = code.replace(/bg-emerald-50 text-emerald-700/g, 'bg-emerald-900/50 text-emerald-300');
    ...
    // Fallbacks
    code = code.replace(/bg-white/g, 'bg-[#0f172a]/80 backdrop-blur-xl');
    
    fs.writeFileSync(filePath, code);
}

replaceTheme('src/components/BiblePanel.tsx');
replaceTheme('src/components/ToolsPanel.tsx');

// Also fix App.tsx's light classes where currentSection !== 'home'
let appCode = fs.readFileSync('src/App.tsx', 'utf-8');
appCode = appCode.replace(/bg-white\/60 border-slate-200\/50/g, 'bg-[#0f172a]/60 border-slate-700/50');
appCode = appCode.replace(/text-slate-800 hover:text-indigo-600/g, 'text-slate-200 hover:text-indigo-400');
appCode = appCode.replace(/text-indigo-600/g, 'text-indigo-400');
appCode = appCode.replace(/text-slate-500 hover:bg-slate-100 hover:text-slate-800/g, 'text-slate-400 hover:bg-slate-800 hover:text-slate-200');
