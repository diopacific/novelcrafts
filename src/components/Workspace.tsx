              ...
              {!draftScenes ? (
                <div className="flex justify-end pt-2">
                  <Button 
                    ...
                    className="w-full sm:w-auto text-[15px] px-8 py-6 h-auto shadow-md"
                  >
                    {planning ? (
                      <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-3"/> 장면 분할 및 플롯 기획 중...</>
                    ) : (
                      <><Layers className="w-5 h-5 mr-2" /> 4장면 플롯 분할하기</>
                    )}
                  </Button>
                </div>
              ) : (
                <div className="pt-8 border-t border-indigo-100 mt-8 space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-bold text-slate-800 flex items-center gap-2">
                        <Layers className="w-5 h-5 text-indigo-500" /> 세부 장면 단위 집필
                      </h4>
                      <p className="text-[13px] text-slate-500 mt-1">각 장면의 플롯을 수정하고 개별적으로 집필하여 5,000자 분량을 확보하세요.</p>
                    </div>
                    ...
                    {draftScenes.map((scene, index) => (
                      <div key={scene.id} className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                        <div className="px-5 py-3 border-b border-slate-200 bg-white flex items-center justify-between">
                          ...
                          {scene.status === 'done' && <span className="flex items-center text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-200"><Check className="w-3 h-3 mr-1" /> 작성 완료</span>}
                        </div>
                        <div className="p-5 space-y-4">
                          <div className="space-y-2">
                            <label className="text-[12px] font-bold text-slate-500 uppercase">장면 플롯</label>
                            <Textarea 
                              className="h-24 text-[14px] bg-white border-slate-200 resize-none"
                              value={scene.plot}
                             ...
                             <div className="flex justify-end">
                                ...
                                >
                                  {scene.status === 'generating' ? (
                                    <><div className="w-4 h-4 border-2 border-slate-400/30 border-t-slate-600 rounded-full animate-spin mr-2"/> 이 장면 집필 중...</>
                                  ) : (
                                    <><Play className="w-4 h-4 mr-1.5" /> 1,500자 분량 집필</>
                                  )}
                                </Button>
                             </div>
                          )}
                  ...
                  <div className="pt-6 flex justify-end">
                    <Button 
                      ...
                      disabled={summarizing || draftScenes.some(s => s.status !== 'done')}
                      className="w-full sm:w-auto text-[15px] px-8 py-6 h-auto shadow-md"
                    >
                      {summarizing ? (
                        <><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-3"/> 요약 및 회차 완성 중...</>
                      ) : (
                        <><CheckCircle2 className="w-5 h-5 mr-2" /> 모든 장면 병합하여 1화 완성하기</>
                      )}
                    </Button>
                  </div>
                </div>
